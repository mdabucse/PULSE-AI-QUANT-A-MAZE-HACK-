...
def only_face():
    ...
    face_cascade = cv2.CascadeClassifier('A:\Projects\Vital Monitoring\From_Scratch\haarcascade_frontalface_default.xml')
        ...
        if ret == True:
            ...
            for (x,y,w,h) in faces:
                ...
                cv2.putText(output, "{} C".format(temperature), (x, y-5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2, cv2.LINE_AA)
                print(temperature)
                save_to_file("Data/temperature.txt", temperature)
